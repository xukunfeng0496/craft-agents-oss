...
## How it Works (Video)
To understand what Work Agents does and how it works watch this video.
...
## Why Work Agents was built
Work Agents is a tool we built so that we (at craft.do) can work effectively with agents. It enables intuitive multitasking, no-fluff connection to any API or Service, sharing sessions, and a more document (vs code) centric workflow - in a beautiful and fluid UI.
...
Work Agents is open source under the Apache 2.0 license - so you are free to remix, change anything. And that's actually possible. We ourselves are building Work Agents with Work Agents only - no code editors - so really, any customisation is just a prompt away.

We built Work Agents because we wanted a better, more opinionated (and preferably non-CLI way) of working with the most powerful agents in the world. We'll continue to improve it, based on our experiences and intuition.
...
Work Agents connects to anything. We have it hooked up to a direct Postgres DB behind a jumpbox. Skills + Sources = magic.
...
# Debug logging (writes to ~/Library/Logs/Work Agents/)
...
4. Name: e.g., "Work Agent Desktop"
5. Click **Create**
6. Note the **Client ID** and **Client Secret**

#### 5. Configure in Work Agent
...
Work Agents uses two different agent backends:
...
Configuration is stored at `~/.workagent/`:
...
Or configure manually in `~/.workagent/workspaces/{id}/hooks.json`:
    ...
    "LabelAdd": [
      {
        ...
        "hooks": [
          { "type": "command", "command": "osascript -e 'display notification \"Urgent session\" with title \"Work Agent\"'" }
        ]
      }
    ]
  }
...
External apps can navigate using `workagents://` URLs:

```
workagents://allSessions                      # All sessions view
workagents://allSessions/session/session123   # Specific session
workagents://settings                         # Settings
workagents://sources/source/github            # Source info
workagents://action/new-chat                  # Create new session
```
...
| AI (OpenAI) | Work Agents Codex fork (app-server) |
...
**macOS:**
```bash
/Applications/Work\ Agents.app/Contents/MacOS/Work\ Agents -- --debug
```

**Windows (PowerShell):**
```powershell
& "$env:LOCALAPPDATA\Programs\@work-agent\electron\Work Agents.exe" -- --debug
```
...
Logs are written to:
- **macOS:** `~/Library/Logs/@work-agent/electron/main.log`
- **Windows:** `%APPDATA%\@work-agent\electron\logs\main.log`
- **Linux:** `~/.config/@work-agent/electron/logs/main.log`
...
Work Agents also bundles a custom Codex app-server fork to support OpenAI/Codex connections:

- https://github.com/lukilabs/craft-agents-codex

### Trademark

"Craft" and "Work Agents" are trademarks of Craft Docs Ltd. See [TRADEMARK.md](TRADEMARK.md) for usage guidelines.
