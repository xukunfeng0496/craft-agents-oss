...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in Work Agent.
...
- `~/.workagent/workspaces/{workspaceId}/sources/{sourceSlug}/`
...
`~/.workagent/provider-domains.json`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.workagent/workspaces/{ws}/sources/my-source
   ```
