...
This guide explains how to customize the visual theme of Work Agent.

## Overview

Work Agent uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.workagent/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.workagent/theme.json` - Override specific colors (app-level)
...
```
~/.workagent/workspaces/{id}/config.json
```
...
Create `~/.workagent/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.workagent/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.workagent/themes/{name}.json`
...
1. Create `~/.workagent/theme.json` for overrides or `~/.workagent/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.workagent/theme.json` for overrides, `~/.workagent/themes/` for presets)
