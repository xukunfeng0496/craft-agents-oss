...
When your OAuth token expires, Work Agent automatically refreshes it in the background using the refresh token. No manual re-authentication required — your MCP connections stay alive.
...
Configure sources that require multiple authentication headers (e.g., API key + tenant ID). See the updated documentation at `~/.workagent/docs/sources.md`.
