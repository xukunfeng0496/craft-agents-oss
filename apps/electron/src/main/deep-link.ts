 ...
 * Deep Link Handler
 *
 * Parses workagents:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   workagents://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   workagents://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   workagents://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   workagents://sources[/source/{sourceSlug}]          - Sources list
 *   workagents://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   workagents://action/{actionName}[/{id}][?params]
 *   workagents://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *   workagents://allSessions/session/abc123                (specific session)
 *   workagents://settings/shortcuts                     (shortcuts page)
 *   workagents://sources/source/github                  (github source info)
 *   workagents://action/new-chat                        (uses active window)
 *   workagents://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   workagents://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., workagents://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., workagents://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // workagents://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // workagents://allSessions/..., workagents://settings/..., etc. (compound routes)
    ...
    // workagents://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // workagents://action/... (no workspace - uses active window)
    if (host === 'action') {
