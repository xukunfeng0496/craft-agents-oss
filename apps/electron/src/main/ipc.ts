...
async function fetchAndStoreCopilotModels(slug: string, accessToken: string): Promise<void> {
  ...
  const writeDebugFile = async () => {
    try {
      const debugPath = join(homedir(), '.workagent', 'copilot-debug.log')
...
export function registerIpcHandlers(sessionManager: SessionManager, windowManager: WindowManager, browserPaneManager: BrowserPaneManager | null): void {
  ...
  ipcMain.handle(IPC_CHANNELS.CHECK_WORKSPACE_SLUG, async (_event, slug: string) => {
    const defaultWorkspacesDir = join(homedir(), '.workagent', 'workspaces')
  ...
  ipcMain.handle(IPC_CHANNELS.OPEN_SESSION_IN_NEW_WINDOW, async (_event, workspaceId: string, sessionId: string) => {
    ...
    const deepLink = `workagents://allSessions/session/${sessionId}`
  ...
  // Manual checks from renderer should behave like a normal app updater:
  // once an update is found, start downloading immediately.
  ...
  ipcMain.handle(IPC_CHANNELS.UPDATE_DISMISS, async (_event, version: string) => {
    const { setDismissedUpdateVersion } = await import('@work-agent/shared/config')
    setDismissedUpdateVersion(version)
  })

  // Get dismissed version
  ipcMain.handle(IPC_CHANNELS.UPDATE_GET_DISMISSED, async () => {
    const { getDismissedUpdateVersion } = await import('@work-agent/shared/config')
  ...
  ipcMain.handle(IPC_CHANNELS.OPEN_URL, async (_event, url: string) => {
    ...
    try {
      ...
      // Handle workagents:// URLs internally via deep link handler
      // This ensures ?window= params work correctly for "Open in New Window"
      if (parsed.protocol === 'workagents:') {
  ...
  ipcMain.handle(IPC_CHANNELS.LOGOUT, async () => {
    try {
      ...
      const configPath = join(homedir(), '.workagent', 'config.json')
  ...
  ipcMain.handle(IPC_CHANNELS.SETTINGS_TEST_API_CONNECTION, async (_event, apiKey: string, baseUrl?: string, models?: string[]): Promise<{ success: boolean; error?: string; modelCount?: number }> => {
    ...
    try {
      ...
      if (isToolSupportError) {
        ...
        return { success: false, error: `Model "${displayModel}" does not support tool/function calling. Work Agent requires a model with tool support (e.g. Claude, GPT-4, Gemini).` }
      }
  ...
  ipcMain.handle(IPC_CHANNELS.WORKSPACE_SETTINGS_GET, async (_event, workspaceId: string) => {
    ...
    const { loadWorkspaceConfig } = await import('@work-agent/shared/workspaces')
  ...
  ipcMain.handle(IPC_CHANNELS.WORKSPACE_SETTINGS_UPDATE, async (_event, workspaceId: string, key: string, value: unknown) => {
    ...
    const { loadWorkspaceConfig, saveWorkspaceConfig } = await import('@work-agent/shared/workspaces')
    const config = loadWorkspaceConfig(workspace.rootPath)
    if (!config) {
  ...
  ipcMain.handle(IPC_CHANNELS.LLM_CONNECTION_TEST, async (_event, slug: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (isOpenAiProvider && connection.authType === 'oauth') {
        ...
        try {
          const { refreshChatGptTokens } = await import('@work-agent/shared/auth/chatgpt-oauth')
      ...
      if (isAnthropicProvider && connection.authType === 'oauth') {
        const { getValidClaudeOAuthToken } = await import('@work-agent/shared/auth/state')
  ...
  ipcMain.handle(IPC_CHANNELS.CHATGPT_START_OAUTH, async (_event, connectionSlug: string): Promise<{
    ...
    try {
      const { startChatGptOAuth, exchangeChatGptCode } = await import('@work-agent/shared/auth')
      const credentialManager = getCredentialManager()
  ...
  ipcMain.handle(IPC_CHANNELS.CHATGPT_CANCEL_OAUTH, async (): Promise<{ success: boolean }> => {
    try {
      const { cancelChatGptOAuth } = await import('@work-agent/shared/auth')
      cancelChatGptOAuth()
  ...
  ipcMain.handle(IPC_CHANNELS.COPILOT_START_OAUTH, async (event, connectionSlug: string): Promise<{
    ...
    try {
      const { startGithubOAuth } = await import('@work-agent/shared/auth')
      const credentialManager = getCredentialManager()
  ...
  ipcMain.handle(IPC_CHANNELS.COPILOT_CANCEL_OAUTH, async (): Promise<{ success: boolean }> => {
    try {
      const { cancelGithubOAuth } = await import('@work-agent/shared/auth')
      cancelGithubOAuth()
  ...
  // Create a new source
  ipcMain.handle(IPC_CHANNELS.SOURCES_CREATE, async (_event, workspaceId: string, config: Partial<import('@work-agent/shared/sources').CreateSourceInput>) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { createSource } = await import('@work-agent/shared/sources')
  ...
  ipcMain.handle(IPC_CHANNELS.SOURCES_DELETE, async (_event, workspaceId: string, sourceSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { deleteSource } = await import('@work-agent/shared/sources')
  ...
  ipcMain.handle(IPC_CHANNELS.SOURCES_START_OAUTH, async (_event, workspaceId: string, sourceSlug: string) => {
    try {
      ...
      const { loadSource, getSourceCredentialManager } = await import('@work-agent/shared/sources')
  ...
  ipcMain.handle(IPC_CHANNELS.SOURCES_GET_PERMISSIONS, async (_event, workspaceId: string, sourceSlug: string) => {
    ...
    const { getSourcePermissionsPath } = await import('@work-agent/shared/agent')
  ...
  ipcMain.handle(IPC_CHANNELS.WORKSPACE_GET_PERMISSIONS, async (_event, workspaceId: string) => {
    ...
    // Load raw JSON file (not normalized) for UI display
    const { existsSync, readFileSync } = await import('fs')
    const { getWorkspacePermissionsPath } = await import('@work-agent/shared/agent')
  ...
  // Get default permissions from ~/.workagent/permissions/default.json
  ...
  ipcMain.handle(IPC_CHANNELS.DEFAULT_PERMISSIONS_GET, async () => {
    const { existsSync, readFileSync } = await import('fs')
    const { getAppPermissionsDir } = await import('@work-agent/shared/agent')
  ...
  ipcMain.handle(IPC_CHANNELS.SOURCES_GET_MCP_TOOLS, async (_event, workspaceId: string, sourceSlug: string) => {
    ...
    try {
      ...
      const { CraftMcpClient } = await import('@work-agent/shared/mcp')
      ...
      const { loadSourcePermissionsConfig, permissionsConfigCache } = await import('@work-agent/shared/agent')
  ...
  ipcMain.handle(IPC_CHANNELS.SEARCH_SESSIONS, async (_event, workspaceId: string, query: string, searchId?: string) => {
    ...
    const { getWorkspaceSessionsPath } = await import('@work-agent/shared/workspaces')
  ...
  ipcMain.handle(IPC_CHANNELS.SKILLS_DELETE, async (_event, workspaceId: string, skillSlug: string) => {
    ...
    const { deleteSkill } = await import('@work-agent/shared/skills')
  ...
  ipcMain.handle(IPC_CHANNELS.STATUSES_LIST, async (_event, workspaceId: string) => {
    ...
    const { listStatuses } = await import('@work-agent/shared/statuses')
  ...
  ipcMain.handle(IPC_CHANNELS.LABELS_LIST, async (_event, workspaceId: string) => {
    ...
    const { listLabels } = await import('@work-agent/shared/labels/storage')
    return listLabels(workspace.rootPath)
  })

  // Create a new label in a workspace
  ipcMain.handle(IPC_CHANNELS.LABELS_CREATE, async (_event, workspaceId: string, input: import('@work-agent/shared/labels').CreateLabelInput) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const { createLabel } = await import('@work-agent/shared/labels/crud')
  ...
  ipcMain.handle(IPC_CHANNELS.LABELS_DELETE, async (_event, workspaceId: string, labelId: string) => {
    ...
    const { deleteLabel } = await import('@work-agent/shared/labels/crud')
  ...
  ipcMain.handle(IPC_CHANNELS.VIEWS_LIST, async (_event, workspaceId: string) => {
    ...
    const { listViews } = await import('@work-agent/shared/views/storage')
    return listViews(workspace.rootPath)
  })

  // Save views (replaces full array)
  ipcMain.handle(IPC_CHANNELS.VIEWS_SAVE, async (_event, workspaceId: string, views: import('@work-agent/shared/views').ViewConfig[]) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const { saveViews } = await import('@work-agent/shared/views/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.THEME_GET_WORKSPACE_COLOR_THEME, async (_event, workspaceId: string) => {
    const { getWorkspaces } = await import('@work-agent/shared/config/storage')
    const { getWorkspaceColorTheme } = await import('@work-agent/shared/workspaces/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.THEME_SET_WORKSPACE_COLOR_THEME, async (_event, workspaceId: string, themeId: string | null) => {
    const { getWorkspaces } = await import('@work-agent/shared/config/storage')
    const { setWorkspaceColorTheme } = await import('@work-agent/shared/workspaces/storage')
    const workspaces = getWorkspaces()
    const workspace = workspaces.find(w => w.id === workspaceId)
    if (!workspace) return
  ...
  ipcMain.handle(IPC_CHANNELS.TOOL_ICONS_GET_MAPPINGS, async () => {
    const { getToolIconsDir } = await import('@work-agent/shared/config/storage')
    const { loadToolIconConfig } = await import('@work-agent/shared/utils/cli-icon-resolver')
    const { encodeIconToDataUrl } = await import('@work-agent/shared/utils/icon-encoder')
    const { join } = await import('path')
  ...
  ipcMain.handle(IPC_CHANNELS.LOGO_GET_URL, async (_event, serviceUrl: string, provider?: string) => {
    const { getLogoUrl } = await import('@work-agent/shared/utils/logo')
  ...
  ipcMain.handle(IPC_CHANNELS.NOTIFICATION_GET_ENABLED, async () => {
    const { getNotificationsEnabled } = await import('@work-agent/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.NOTIFICATION_SET_ENABLED, async (_event, enabled: boolean) => {
    const { setNotificationsEnabled } = await import('@work-agent/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.INPUT_GET_AUTO_CAPITALISATION, async () => {
    const { getAutoCapitalisation } = await import('@work-agent/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.INPUT_SET_AUTO_CAPITALISATION, async (_event, enabled: boolean) => {
    const { setAutoCapitalisation } = await import('@work-agent/shared/config/storage')
    setAutoCapitalisation(enabled)
  })

  // Get send message key setting
  ipcMain.handle(IPC_CHANNELS.INPUT_GET_SEND_MESSAGE_KEY, async () => {
    const { getSendMessageKey } = await import('@work-agent/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.INPUT_SET_SEND_MESSAGE_KEY, async (_event, key: 'enter' | 'cmd-enter') => {
    const { setSendMessageKey } = await import('@work-agent/shared/config/storage')
    setSendMessageKey(key)
  })

  // Get spell check setting
  ipcMain.handle(IPC_CHANNELS.INPUT_GET_SPELL_CHECK, async () => {
    const { getSpellCheck } = await import('@work-agent/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.INPUT_SET_SPELL_CHECK, async (_event, enabled: boolean) => {
    const { setSpellCheck } = await import('@work-agent/shared/config/storage')
    setSpellCheck(enabled)
  })

  // Get keep awake while running setting
  ipcMain.handle(IPC_CHANNELS.POWER_GET_KEEP_AWAKE, async () => {
    const { getKeepAwakeWhileRunning } = await import('@work-agent/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.POWER_SET_KEEP_AWAKE, async (_event, enabled: boolean) => {
    const { setKeepAwakeWhileRunning } = await import('@work-agent/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.APPEARANCE_GET_RICH_TOOL_DESCRIPTIONS, async () => {
    const { getRichToolDescriptions } = await import('@work-agent/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.APPEARANCE_SET_RICH_TOOL_DESCRIPTIONS, async (_event, enabled: boolean) => {
    const { setRichToolDescriptions } = await import('@work-agent/shared/config/storage')
