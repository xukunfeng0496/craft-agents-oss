 ...
 *
 * Watched paths:
 * - ~/.workagent/config.json - Main app configuration
 * - ~/.workagent/preferences.json - User preferences
 * - ~/.workagent/theme.json - App-level theme overrides
 * - ~/.workagent/themes/*.json - Preset theme files (app-level)
 * - ~/.workagent/workspaces/{slug}/ - Workspace directory (recursive)
...
import { debug, perf } from '@work-agent/shared/utils';
import { readJsonFileSync } from '@work-agent/shared/utils/files';
import { loadStoredConfig, type StoredConfig } from '@work-agent/shared/config';
import {
  validateConfig,
  validatePreferences,
  validateSource,
...
import type { LoadedSource, SourceGuide } from '@work-agent/shared/sources';
import {
  loadSource,
  ...
  downloadSourceIcon,
} from '@work-agent/shared/sources';
import { permissionsConfigCache, getAppPermissionsDir } from '@work-agent/shared/agent';
import { getWorkspacePath, getWorkspaceSourcesPath, getWorkspaceSkillsPath } from '@work-agent/shared/workspaces';
import type { LoadedSkill } from '@work-agent/shared/skills';
import { loadSkill, loadAllSkills, skillNeedsIconDownload, downloadSkillIcon } from '@work-agent/shared/skills';
import {
  loadStatusConfig,
  statusNeedsIconDownload,
  downloadStatusIcon,
} from '@work-agent/shared/statuses';
import { loadAppTheme, loadPresetThemes, loadPresetTheme, getAppThemesDir } from '@work-agent/shared/config';
import type { ThemeOverrides, PresetTheme } from '@work-agent/shared/config';
...
export interface ConfigWatcherCallbacks {
  ...
  /** Called when app-level default permissions change (~/.workagent/permissions/default.json) */
...
export class ConfigWatcher {
  ...
  /**
   * Watch app-level themes directory (~/.workagent/themes/)
   */
  ...
  /**
   * Watch app-level permissions directory (~/.workagent/permissions/)
