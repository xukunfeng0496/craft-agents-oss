...
describe('right sidebar runtime labels', () => {
  it('maps coming-soon panel labels', () => {
    const t: TFunction = ((key: string) => ({
      'common:rightSidebar.files.comingSoon': 'Files panel localized',
      'common:rightSidebar.history.comingSoon': 'History panel localized',
    } as Record<string, string>)[key] || key) as TFunction

    const labels = getRightSidebarLabels(t)
    expect(labels.filesComingSoon).toBe('Files panel localized')
    expect(labels.historyComingSoon).toBe('History panel localized')
  })

  it('maps metadata panel labels', () => {
    const t: TFunction = ((key: string) => ({
