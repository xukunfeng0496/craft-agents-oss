...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  const remoteUrl = session?.remoteUrl || sessionMeta?.remoteUrl || null
  ...
  const handleOpenInNewWindow = React.useCallback(async () => {
    ...
    const url = `workagents://${route}${separator}window=focused`
    try {
  ...
  const handleShare = React.useCallback(async () => {
    ...
    if (result?.success && result.url) {
      ...
      toast.success(t('chat:toast.linkCopied'), {
        description: result.url,
        action: { label: t('chat:actions.open'), onClick: () => window.electronAPI.openUrl(result.url!) },
      })
    } else {
      toast.error(t('chat:toast.shareFailed'), { description: result?.error || t('chat:toast.shareUnknownError') })
    }
  }, [sessionId, t])
  ...
  const handleUpdateShare = React.useCallback(async () => {
    ...
    if (result?.success) {
      toast.success(t('chat:toast.shareUpdated'))
    } else {
      toast.error(t('chat:toast.updateShareFailed'), { description: result?.error })
    }
  }, [sessionId, t])
  ...
  const handleRevokeShare = React.useCallback(async () => {
    ...
    if (result?.success) {
      toast.success(t('chat:toast.shareStopped'))
    } else {
      toast.error(t('chat:toast.stopShareFailed'), { description: result?.error })
    }
  }, [sessionId, t])
        ...
        {sharedUrl ? (
          ...
          <>
            <StyledDropdownMenuItem onClick={handleShare}>
              ...
              <span className="flex-1">{t('chat:share.shareOnline')}</span>
            </StyledDropdownMenuItem>
            <StyledDropdownMenuSeparator />
            <StyledDropdownMenuItem onClick={() => window.electronAPI.openUrl('https://agents.craft.do/docs/go-further/sharing')}>
              <Info className="h-3.5 w-3.5" />
  ...
  if (!session) {
    ...
    return (
      <div className="h-full flex flex-col">
        <PanelHeader  title={t('chat:panel.title')} rightSidebarButton={rightSidebarButton} />
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground">
          <AlertCircle className="h-10 w-10" />
          <p className="text-sm">{t('chat:session.missing')}</p>
        </div>
      </div>
    )
  }
