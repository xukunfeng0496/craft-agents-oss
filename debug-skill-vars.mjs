 ...
 * Usage: node debug-skill-vars.mjs <workspaceId> <skillSlug>
 */

import { getSkillVars } from './packages/shared/dist/skills/vars-storage.js';
import { loadSkillBySlug } from './packages/shared/dist/skills/storage.js';
import { substituteSkillVars } from './packages/shared/dist/skills/vars-substitution.js';
...
async function main() {
  ...
  if (!workspaceId || !skillSlug) {
    console.error('Usage: node debug-skill-vars.mjs <workspaceId> <skillSlug>');
    process.exit(1);
  }
  ...
  // Load skill
  const workspaceRoot = `${homedir()}/.craft-agent/workspaces/${workspaceId}`;
