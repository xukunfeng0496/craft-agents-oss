...
This file provides guidance to Claude Code when working with the `@work-agent/core` package.
...
The core package provides shared TypeScript types and utilities used by the Electron app and shared packages. It serves as the type definition layer for the Work Agent monorepo.
...
| `AgentEvent` | Events from WorkAgent during chat (incl. task_backgrounded, shell_backgrounded, task_progress) |
...
import type {
  ...
  Message,
  AgentEvent,
} from '@work-agent/core';

// Import utilities
import { generateMessageId, debug } from '@work-agent/core';

// Or import from specific subpaths
import type { Session } from '@work-agent/core/types';
```
...
2. **Phase 2:** Move storage logic to `@work-agent/core`
