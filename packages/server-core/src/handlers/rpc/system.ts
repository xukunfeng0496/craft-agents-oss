...
function parseInternalCraftAgentsDeepLink(parsed: URL): ParsedInternalDeepLink | null {
  if (parsed.protocol !== 'workagents:') return null
    ...
    try {
      ...
      if (classification.kind === 'internal-deeplink') {
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing workagents:// URL internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for workagents:// URL')
