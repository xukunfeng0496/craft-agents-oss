...
`@work-agent/shared` is the core business logic package for Work Agent. It contains:
- Agent implementation (WorkAgent, session-scoped tools, permission modes)
...
```typescript
import { CraftAgent, getPermissionMode, setPermissionMode } from '@work-agent/shared/agent';
import { loadStoredConfig, type Workspace } from '@work-agent/shared/config';
import { getCredentialManager } from '@work-agent/shared/credentials';
import { CraftMcpClient } from '@work-agent/shared/mcp';
import { loadWorkspaceSources, type LoadedSource } from '@work-agent/shared/sources';
import { loadStatusConfig, createStatus } from '@work-agent/shared/statuses';
import { resolveTheme } from '@work-agent/shared/config/theme';
...
```
src/
├── agent/              # WorkAgent, session-scoped-tools, mode-manager, mode-types, permissions-config
...
### WorkAgent (`src/agent/craft-agent.ts`)
...
- Workspace: `~/.workagent/workspaces/{id}/permissions.json`
- Source: `~/.workagent/workspaces/{id}/sources/{slug}/permissions.json`
...
**Storage:** `~/.workagent/workspaces/{id}/statuses/config.json`
...
**Storage:**
- App: `~/.workagent/theme.json`
- Workspace: `~/.workagent/workspaces/{id}/theme.json`
...
All sensitive credentials (API keys, OAuth tokens) are stored in an AES-256-GCM encrypted file at `~/.workagent/credentials.enc`. The `CredentialManager` provides the API for reading and writing credentials.
...
│     └── ~/.workagent/workspaces/{ws}/sources/{slug}/          │
└─────────────────────────────────────────────────────────────────┘
                              │
...
Multi-workspace configuration stored in `~/.workagent/config.json`. Supports:
...
### Sources (`src/sources/`)
Sources are external data connections (MCP servers, APIs, local filesystems). Stored at `~/.workagent/workspaces/{id}/sources/{slug}/` with config.json and guide.md. Types: `mcp`, `api`, `local`, `gmail`.

## Dependencies

- `@work-agent/core` - Shared types
