...
class ModeManager {
  ...
  setPermissionMode(sessionId: string, mode: PermissionMode): void {
    ...
    // Notify callbacks (for WorkAgent internal sync)
...
export function getPathHint(targetPath: string, plansFolderPath: string, dataFolderPath?: string): string | null {
  ...
  if (normalizedTarget.includes('/.workagent/workspaces/') && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans or data folder, not the workspace root.';
  }

  // Case: Writing outside .workagent entirely
  if (!normalizedTarget.includes('/.workagent/')) {
...
export function shouldAllowToolInMode(
  ...
  if (toolName.startsWith('mcp__')) {
    ...
    if (toolName.startsWith('mcp__preferences__') || toolName.startsWith('mcp__work-agents-docs__')) {
      return { allowed: true };
    }
