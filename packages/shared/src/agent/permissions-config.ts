 ...
 *
 * File locations:
 * - Workspace: ~/.workagent/workspaces/{slug}/permissions.json
 * - Per-source: ~/.workagent/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.workagent/permissions/
 * Reads env var dynamically so tests can override via CRAFT_CONFIG_DIR.
 */
export function getAppPermissionsDir(): string {
  const configDir = process.env.CRAFT_CONFIG_DIR || join(homedir(), '.workagent');
...
/**
 * Load default permissions from ~/.workagent/permissions/default.json
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.workagent/permissions/default.json)
   ...
   * These come from ~/.workagent/permissions/default.json
   */
