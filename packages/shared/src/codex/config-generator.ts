 ...
 *
 * Generates config.toml files from Work Agent sources for per-session Codex configuration.
...
export function generateCodexConfig(options: CodexConfigGeneratorOptions): CodexConfigResult {
  ...
  // Header comment
  sections.push('# Generated by Work Agent - DO NOT EDIT');
  ...
  // Work Agents documentation - public Mintlify MCP server, no auth needed.
  // Provides SearchWorkAgents tool for finding source setup guides.
  // This matches the work-agents-docs server in Claude's SDK options.
  sections.push('# Work Agents documentation (source setup guides, API references)');
  sections.push('[mcp_servers.work-agents-docs]');
