#!/usr/bin/env bun
/**
 * Debug script to print the full Work Agent system prompt with annotations.
...
printAnnotation('  2. Work Agent Environment Marker - version, platform, arch');
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@work-agent/electron/main.log' },
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// Work Agent additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. Work Agent Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
