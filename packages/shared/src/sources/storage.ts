 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.workagent/workspaces/xxx)
 ...
 * (like work-agents-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only work-agents-docs is a builtin source
      if (slug === 'work-agents-docs') {
 ...
 * Built-in sources (like work-agents-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search work-agents-docs MCP for service-specific guidance)
