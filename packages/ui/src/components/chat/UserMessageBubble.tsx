...
import type { StoredAttachment, ContentBadge } from '@work-agent/core'
import { normalizePath } from '@work-agent/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .workagent workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.workagent\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
