...
// Detects instance number from folder name suffix (e.g., work-agents-1 → instance 1)
function detectInstance(): void {
  ...
  if (match) {
    ...
    process.env.CRAFT_APP_NAME = `Work Agents [${instanceNum}]`;
    process.env.CRAFT_CONFIG_DIR = join(process.env.HOME || "", `.workagent-${instanceNum}`);
    process.env.CRAFT_DEEPLINK_SCHEME = `workagents${instanceNum}`;
...
function getElectronEnv(): Record<string, string> {
  ...
  return {
    ...
    CRAFT_APP_NAME: process.env.CRAFT_APP_NAME || "Work Agents",
    CRAFT_DEEPLINK_SCHEME: process.env.CRAFT_DEEPLINK_SCHEME || "workagents",
