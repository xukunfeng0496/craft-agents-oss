...
GITHUB_REPO="xukunfeng0496/work-agents-oss"
GITHUB_API_URL="https://api.github.com/repos/${GITHUB_REPO}/releases/latest"
DOWNLOAD_DIR="$HOME/.workagent/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="Work-Agent-x64.AppImage"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.workagent.app"
    if pgrep -x "Work Agents" >/dev/null 2>&1; then
        info "Quitting Work Agents..."
        ...
        if pgrep -x "Work Agents" >/dev/null 2>&1; then
            ...
            pkill -9 -x "Work Agents" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  Work Agents has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'Work Agents'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.workagent/app"
    WRAPPER_PATH="$INSTALL_DIR/work-agents"
    APPIMAGE_INSTALL_PATH="$APP_DIR/Work-Agent-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Work-Agent.*AppImage" >/dev/null 2>&1; then
        info "Stopping Work Agents..."
        pkill -f "Work-Agent.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# Work Agent launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.workagent/app/Work-Agent-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@work-agent"
ELECTRON_CACHE_ALT="$HOME/.cache/@work-agent"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: Work Agent not found at $APPIMAGE_PATH"
    echo "Reinstall: curl -fsSL https://raw.githubusercontent.com/xukunfeng0496/work-agents-oss/cvte/main/scripts/install-app.sh | bash"
    exit 1
fi
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/Work-Agent-x64.AppImage"
